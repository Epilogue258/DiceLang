...
from .result import ErrorRes, ExprRes, Result
...
class Evaluator:  # 求值器：输入 AST，输出结果（包含中间过程）。
    ...
    def evaluate(self, node: AstNode) -> Result:
        """
        求值器的安全边界：输入 AST，总是返回 Result（永不抛异常）。
        内部调用 eval() 逐步求值，捕获 EvaluatorError 并包装为 ErrorRes。
        """
        try:
            *_, final = self.eval(node)
            return ExprRes(value=final)
