...
class Evaluator:  # 求值器：输入 AST，输出结果（包含中间过程）。
    ...
    def same_family(self, *nodes: AstNode) -> bool:
        """判断一组节点是否属于同一运算家族（例如都是加法、或者都是乘法）。"""
        return True  # TODO
        raise TodoError("Evaluator.same_family")
    ...
    def simplify(self, node: Any) -> AstNode:
        ...
        if all(isinstance(child, NumberNode) for child in simlified_child) and self.same_family(node, *simlified_child):
            return self.fold(node, simlified_child)
        return type(node)(*simlified_attr)
