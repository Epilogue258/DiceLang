...
class Parser:  # TODO 解析器：输入 Token 流，输出 AST（抽象语法树）。
    ...
    def prefix_parse(self, token: Token) -> AstNode:
        match token.type:
            ...
            case _:
                raise ParserError(
                    f"parse_prefix时得到错误的类型： {token.type} 位于 {token.pos}，内容为：「{token.text}」。当前指针Token具体为：{self.current}",
                    token=token,
                )
    ...
    def _parse_delimited(self, *, end: TokenType = TokenType.RPAREN, delimiter: TokenType = TokenType.COMMA) -> list[AstNode]:
...
# (1)+( ：error.ParserError: 表达式不完整，意外到达文件结尾
# ((1)+(2)) : <Parser: ( (( 1 ) + ( 2 )) )>
