...
class Parser:  # TODO 解析器：输入 Token 流，输出 AST（抽象语法树）。
    ...
    def prefix_parse(self, token: Token) -> AstNode:
        match token.type:
            ...
            case TokenType.LPAREN:  # Return: GroupNode | FuncCallNode TODO: 验证函数调用是否可行
                ...
                # 只要遇上（，则设置为1。故而可以确保绑定力大于右括号，中止解析。同时在默认循环下没遇上（是绑定力为0，不中止解析从而报错。
                group = self.parse(1)

                self.expect(
                    TokenType.RPAREN,
                    error=ParserError(f"遇到了意外的Token：“ {self.current} ”而非闭合括号。", token=self.current, pos=self.current.pos),
                ).consume()
                return GroupNode(group)
            case TokenType.EOF:
            ...
            case _:
                raise ParserError(
                    f"parse_prefix时得到错误的类型： {token.type} 位于 {token.pos}，内容为： {token.text}", token=self.current
                )
    ...
    def parse(self, min_bp: int = 0) -> AstNode:  # TODO 测试空输入
        ...
        while True:
            ...
            if infix_bp is None:
                raise ParserError(f"该TOKEN不能当作中缀运算符：{op_token}")

            left_bp, right_bp = infix_bp
            if left_bp < min_bp:
                break

            if self.match(TokenType.LPAREN, TokenType.RPAREN):
                print(f"cur={self.current}, do it!")
                # TODO DEL
...
# 测试结果：
# () : error.ParserError: 括号内不存在任何可被解析的值
# (1) : <Parser: ( 1 )>
# ((1)) : <Parser: ( ( 1 ) )>
# ((1) : error.ParserError: 遇到了意外的Token：“ None ”而非闭合括号。
# (1)) : error.ParserError: 多余了右括号。
# (1)+) ：error.ParserError: parse_prefix时得到错误的类型： ) 位于 4，内容为： )
# (1)+( ：fail: error.TodoError: EOF # TODO：需要确定是否一定是括号未闭合导致了EOF被当作infix。确认后可修改TODO Err为ParserErr; 或者干脆在infix的时候加一个识别
# ((1)+(2)) : <Parser: ( (( 1 ) + ( 2 )) )>
# (1)(2) : error.ParserError: error.ParserError: 遇到了意外的Token：“(”，可能缺少括号间的运算符
# ((1)(2)) : error.ParserError: 遇到了意外的Token：“ ( ”而非闭合括号。
# (1()2) : error.ParserError: 遇到了意外的Token：“ ( ”而非闭合括号。 # TODO 同上
# (1+()2) : error.ParserError: 括号内不存在任何可被解析的值
# ((1)+2 : error.ParserError: 遇到了意外的Token：“ None ”而非闭合括号。
# 1+(2)) : error.ParserError: 多余了右括号。
# (1)+(2)) : error.ParserError: 多余了右括号。
# (1+(2)3) : error.ParserError: 该TOKEN不能当作中缀运算符：3
# (1+2)3 : error.ParserError: 该TOKEN不能当作中缀运算符：3
# (1+(2*3) : error.ParserError: 遇到了意外的Token：“ None ”而非闭合括号。
