...
class Parser:  # TODO 解析器：输入 Token 流，输出 AST（抽象语法树）。
    ...
    def expect(self, *expected_type: TokenType, error: Exception | None = None) -> Self:  # 链式调用试试？
        if self.current.type not in expected_type:
            raise (
                error
                if error
                else SyntaxError(
    ...
    def match(self, *expected_type: TokenType) -> bool:
        return self.current.type in expected_type
    ...
    def prefix_parse(self, token: Token) -> AstNode:
        match token.type:
            ...
            case TokenType.LPAREN:
                ...
                self.expect(TokenType.RPAREN, error=SyntaxError("未找到右括号！")).consume()
                if not group:
                    raise ValueError("括号内不存在任何可被解析的值")
...
infix_parselets: dict[TokenType, tuple[int, int]] = {
    ...
    TokenType.DICE: (60, 61),
}

if __name__ == "__main__":  # TODO 测试用
    sounce = "(-2d(2**6)d8*3**4**4))"
    lexer = Lexer(sounce)
