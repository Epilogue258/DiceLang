...
# 固定case的happy测试
@pytest.mark.parametrize("text, expects", [
    # 基本解析+D的转换为d
    ("12D6", [
        (tktype.NUMBER, 12),
        (tktype.DICE, 'd'),
    ...
    ("<= >= == !=", [
        (tktype.LTE, '<='), (tktype.GTE, '>='),
        (tktype.EQ, '=='), (tktype.NEQ, '!='),
    ]),
        ("< = > =", [  # 分开写的运算符
        (tktype.LT, '<'), (tktype.ASSIGN, '='),
        (tktype.GT, '>'), (tktype.ASSIGN, '='),
    ]),
    ("1-2", [  # 减号
        (tktype.NUMBER, 1), (tktype.MINUS, '-'), (tktype.NUMBER, 2),
    ]),
    ("1--2", [  # 连续减号（可能用于表示负数？词法阶段应输出两个 MINUS）
        (tktype.NUMBER, 1), (tktype.MINUS, '-'), (tktype.MINUS, '-'), (tktype.NUMBER, 2),
    ]),
...
def test_lex_happy(text, expects: list[tuple[tktype, Any]]):
    ...
    for token, (type, value) in zip(lexer.tokens, expects): # EOF会被zip自动截断, 无需处理
